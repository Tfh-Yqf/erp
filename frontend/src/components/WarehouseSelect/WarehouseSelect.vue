...
<script>
  import { warehouseList } from '@/api/manage'
  ...
  export default {
    ...
    props:{
      ...
      value:{
        type:Number,
        default : window.localStorage.getItem('warehouse')
      },
    },
    ...
    computed: {
      options() {
        ...
        if (this.defaultItem && !this.loading && (!this.searchForm.search || this.searchForm.search == '')) {
          if (this.defaultItem.warehouse && this.defaultItem.warehouse_name) {
            if (this.items.findIndex(item => item.id == this.value) == -1) {
              items.splice(0, 0, {id: this.defaultItem.warehouse, name: this.defaultItem.warehouse_name});
            }
          }
        }
        return items
    ...
    methods: {
      list() {
        ...
        this.loading = true;
        warehouseList({...this.searchForm, is_active: true})
          .then(resp => {
