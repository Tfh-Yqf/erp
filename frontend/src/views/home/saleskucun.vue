  ...
  <div>
    <a-card style="height: calc(50vh - 40px);">
      <div slot="title">
        ...
        <div id="saleskucun" style="margin-top: 36px;"></div>
      </div>
    </a-card>
  </div>
</template>
...
export default {
  ...
  methods: {
    initialize() {
      ...
      this.chart = new window.G2.Chart({ container: 'saleskucun', autoFit: true, height, });
    ...
    list() {
      ...
      this.loading = true;
      kucun(form).then(resp => {
        console.log('saleskucun');
        console.log(resp);
